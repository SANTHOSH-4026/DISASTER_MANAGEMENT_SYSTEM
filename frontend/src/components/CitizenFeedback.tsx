...
export function CitizenFeedback() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { 
      id: '1', 
      type: 'bot', 
      content: 'Namaste! I\'m your India Disaster Management Assistant. How can I help you today? In an emergency, please dial 112 (National Emergency), 1078 (Disaster Management), or 108 (Ambulance).',
      timestamp: new Date()
    },
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const botMessage: Message = {
        ...
        type: 'bot',
        content: data.reply || 'Sorry, I am having trouble connecting.',
        timestamp: new Date()
      };
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="text-slate-200 text-sm">{report.user_name || report.user}</div>
                    ...
                    <div className="flex gap-2">
                      ...
                      {report.status === 'pending' && role === 'admin' && (
                        <Button 
                          onClick={() => handleVerifyReport(report.id)}
                          size="sm" 
                          variant="outline" 
                          className="text-xs h-7 border-green-500/50 text-green-400 hover:bg-green-500/20"
                        >
                          Verify Report
                        </Button>
                      )}
